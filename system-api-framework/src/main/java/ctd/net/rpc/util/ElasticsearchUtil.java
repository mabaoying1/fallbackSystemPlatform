...
public class ElasticsearchUtil {
     ...
     * @return
     */
    public ResponseEntity queryElasticsearchDataByCondition(String index, Map<String,Object> req){
        TransportClient client = null;
...
//                    .get();
            SearchRequestBuilder searchRequestBuilder =client.prepareSearch(index)
                    .setQuery(QueryBuilders.wildcardQuery("methodName", "广东*"))
                  //  .setSize(numPerPage).setFrom(from).setExplain(true)
                    .addSort("InParameterDateTime", SortOrder.DESC);
...
//            SearchHits hits = sResponse.getHits();
            ...
            e.printStackTrace();
            logger.error("Elasticsearch【queryElasticsearchDataByID】数据失败;数据【"+req.toString()+"】");
            return ResultMessage.error(500,e.getMessage());
        }finally {
            closeElasticsearch(client);
        }
